...
class QubitState:
    ...
    def apply_gate(self, target: int, matrix: List[complex], controls: Optional[List[int]] = None, control_value: bool = True) -> None:
        if controls:
            ...
            deactivated.clear()
            for k, v in self.state.items():
                if all(bool(k[c]) == control_value for c in controls):
                    activated.state[k] = v
                else:
    ...
    def apply_two_qubit_gate(self, t1: int, t2: int, matrix: List[List[complex]], controls: Optional[List[int]] = None, control_value: bool = True) -> None:
        if controls:
            activated = QubitState(self.n_qubits)
            activated.clear()
            deactivated = QubitState(self.n_qubits)
    ...
    def apply_three_qubit_gate(self, t1: int, t2: int, t3: int, matrix: List[List[complex]], controls: Optional[List[int]] = None, control_value: bool = True) -> None:
        if controls:
            activated = QubitState(self.n_qubits)
            activated.clear()
            deactivated = QubitState(self.n_qubits)
            ...
            for k, v in self.state.items():
                if all(bool(k[c]) == control_value for c in controls):
                    ...
                    deactivated.state[k] = v
            activated.apply_three_qubit_gate(t1, t2, t3, matrix)
            self.state = activated.state
            for k, v in deactivated.state.items():
                self.state[k] = self.state.get(k, 0) + v
            self.remove_zero_entries()
        ...
        new_state: Dict[StateKey, complex] = {}
        for k, v in self.state.items():
            i = int(k[t1]) + 2*int(k[t2]) + 4*int(k[t3])
            for row in range(8):
                nk = list(k)
                nk[t1] = bool(row & 1)
                nk[t2] = bool((row & 2) >> 1)
    ...
    def apply_four_qubit_gate(self, t1: int, t2: int, t3: int, t4: int, matrix: List[List[complex]], controls: Optional[List[int]] = None, control_value: bool = True) -> None:
        if controls:
            activated = QubitState(self.n_qubits)
            activated.clear()
            deactivated = QubitState(self.n_qubits)
            ...
            for k, v in self.state.items():
                if all(bool(k[c]) == control_value for c in controls):
                    ...
                    deactivated.state[k] = v
            activated.apply_four_qubit_gate(t1, t2, t3, t4, matrix)
            self.state = activated.state
            for k, v in deactivated.state.items():
                self.state[k] = self.state.get(k, 0) + v
            self.remove_zero_entries()
        ...
        new_state: Dict[StateKey, complex] = {}
        for k, v in self.state.items():
            i = int(k[t1]) + 2*int(k[t2]) + 4*int(k[t3]) + 8*int(k[t4])
            for row in range(16):
                nk = list(k)
                nk[t1] = bool(row & 1)
                nk[t2] = bool((row & 2) >> 1)
    ...
    def always_activated(self, indices: List[int], value: bool = True) -> bool:
        return all(all(bool(k[i]) == value for i in indices) for k in self.state)

    def never_activated(self, indices: List[int], value: bool = True) -> bool:
        return all(not all(bool(k[i]) == value for i in indices) for k in self.state)
